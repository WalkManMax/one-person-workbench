...
- 🖥 桌面端自动适配为居中手机框预览
- 🗓️ 考勤日程本：支持在岗、出海、出差、休假、值班（工作日/节假日）、公休、请假；支持按月份/状态/关键词筛选，并以表格或日程本浏览
- ✍️ 每条考勤记录强制补齐工作小结与每日反思；出海/出差/值班按状态动态显示时间、地点、事由或值班类型字段
- 💾 数据安全：默认仅存本机浏览器，可导出完整 JSON 备份或考勤 CSV；飞书同步需先完成授权与字段选择，不会自动上传数据
